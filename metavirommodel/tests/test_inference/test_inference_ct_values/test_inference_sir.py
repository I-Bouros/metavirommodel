...
class TestMVRCtValLogLik(unittest.TestCase):
    """
    Test the 'MVRCtValLogLik' class.
    """
...
class TestPHELogPrior(unittest.TestCase):
    ...
    def test__call__(self):
        ...
        log_prior = mvri.MVRCtValLogPrior(model)

        self.assertIsInstance(log_prior([3, 0.002]), (int, float))
        self.assertEqual(log_prior([3, 0.002]) < 0, True)
...
class TestMVRCtValInfer(unittest.TestCase):
    """
    Test the 'MVRCtValInfer' class.
    """
    def test__init__(self):
        ...
        inference = mvri.MVRCtValInfer(model, generation_times)

        self.assertIsInstance(inference._model, mvr.Metaviromodel)

        with self.assertRaises(TypeError):
            mvr.inference.MVRCtValInfer(0)
    ...
    def test_optimisation_problem_setup(self):
        ...
        # Set up MVRCtVal Inference class
        optimisation = mvri.MVRCtValInfer(model, generation_times)
