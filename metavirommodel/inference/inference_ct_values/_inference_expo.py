...
dynamics model with an exponential birth term for rodents (subject to temporal
...
#
# ExponentialGrowthMVRCtValLogLik Class
#

class ExponentialGrowthMVRCtValLogLik(mvri.MVRCtValLogLik):
    """ExponentialGrowthMVRCtValLogLik Class:
    Controller class to construct the log-likelihood needed for optimisation or
    inference of the MVR model with exponential growth in a PINTS framework.

    Parameters
    ----------
    model : ExponentialGrowthMetaviromodel
    ...
    def __init__(self, model, ct_values_data, parameters_ct, generation_times):
        super().__init__(
            model, ct_values_data, parameters_ct, generation_times)

        # Assign model for inference or optimisation
        if not isinstance(model, mvr.ExponentialGrowthMetaviromodel):
    ...
    def __right_hand_side(self, t, y, c):
        ...
        # Write actual RHS
        dydt = [
            theta(t) * np.sum(y) - beta * np.asarray(
...
class ExponentialMVRCtValLogPrior(pints.LogPrior):
    """ExponentialMVRCtValLogPrior Class:
    ...
    def __init__(self, model):
        super(ExponentialMVRCtValLogPrior, self).__init__()
    ...
    def __call__(self, x):
        ...
        return uniform.logpdf(x[0], 0.5, 10)


#
# ExponentialGrowthMVRCtValInfer Class
#

class ExponentialGrowthMVRCtValInfer(mvri.MVRCtValInfer):
    """ExponentialGrowthMVRCtValInfer Class:
    Controller class for the optimisation or inference of parameters of the
    MVR model with exponential growth in a PINTS framework.
    ...
    def __init__(self, model, generation_times):
        super(ExponentialGrowthMVRCtValInfer, self).__init__(
    ...
    def return_loglikelihood(self, x):
        ...
        """
        loglikelihood = ExponentialGrowthMVRCtValLogLik(
    ...
    def _create_posterior(self):
        ...
        """
        # Create a likelihood
        self.loglikelihood = ExponentialGrowthMVRCtValLogLik(
            self._model, self._ct_values, self._ct_parameters,
            self._generation_times)

        # Create a prior
        log_prior = ExponentialMVRCtValLogPrior(self._model)
